...
コードレベルでは全バージョンで z グリフの構造的欠落は確認できなかった（v0.8〜v1.03 のリリース済み TTF でも `cid09512` に対応するグリフは存在）。
再現を確認するには以下を issue でユーザーに確認する。

- [ ] 使用フォントのバージョン（タグ名）
- [ ] 使用アプリケーション・OS（Mac / Windows / Android / Web）
- [ ] 問題が発生した具体的な漢字・拼音（例: 丈 → zhàng の z が消える）
- [ ] スクリーンショット
- [ ] フォントの適用方法（システムフォント / アプリ内埋め込み など）

特定の環境・アプリでのみ GSUB 置換が効かずに拼音グリフ自体が非表示になるケースが疑われる。
