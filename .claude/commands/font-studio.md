...
1. **前提チェック**（不足があれば手順を案内して中断）
   ...
   ```bash
   scripts/dev-tool.sh uvicorn --version || echo "uvicorn が見つからない"
   ...
   - uvicorn が見つからない場合: `python3.11 -m venv .venv && .venv/bin/pip install -e ".[webapp]"`
     （`scripts/dev-tool.sh` は `.venv` があればそれを、なければ PATH 上のツールを使う。
     pyenv/conda/system python など環境によらず動く）
...
2. **起動**: `.claude/launch.json` に `font-studio` 設定があるので、
   ...
   ```bash
   nohup scripts/dev-tool.sh uvicorn webapp.backend.main:app --port 8000 > /tmp/font-studio.log 2>&1 &
   ```
