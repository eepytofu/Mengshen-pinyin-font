...
1. **前提チェック**（不足があれば手順を案内して中断）
   ```bash
   ls .venv/bin/uvicorn || echo "venv がない"
   which otfccdump otfccbuild jq
   ls webapp/frontend/dist/index.html || echo "フロントエンド未ビルド"
   ```
   - venv がない場合: `/opt/homebrew/bin/python3.11 -m venv .venv && .venv/bin/pip install -e ".[webapp]"`
     ※ この端末のデフォルト `python3`（pyenv 3.12.4）は dyld エラーで壊れているため必ず python3.11 を使う
...
2. **起動**: `.claude/launch.json` に `font-studio` 設定があるので、
   ...
   使えない環境では:
   ```bash
   nohup .venv/bin/uvicorn webapp.backend.main:app --port 8000 > /tmp/font-studio.log 2>&1 &
   ```
...
.venv/bin/uvicorn webapp.backend.main:app --port 8000 --reload
# ターミナル2: frontend
