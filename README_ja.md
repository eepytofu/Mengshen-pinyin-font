...
のフォークです。このフォントは漢字の上に拼音を表示し、多音字の読みを文脈から選び
ます。このフォークは宋体を7ウェイトすべてで生成します。

> [!IMPORTANT]
> これらは公式ビルドではなく、サポートもありません。上流リポジトリには問題を報告
> しないでください。
...
| | 上流 | このフォーク |
| :- | :- | :- |
| 宋体のウェイト | Regular のみ | 7種、ExtraLight から Heavy |
| 略 lüè | `lvè` と表示 | `lüè` と表示（他に24字） |
| Windows のファミリ名 | `Mengshen-Regular` | `Mengshen-HanSerif`、Bold が Regular に連結 |
| 源ノ明朝 | 1.001 | 2.003 |
| ソースフォント | リポジトリに同梱 | スクリプトで取得 |

拼音のウェイトは bold より太くなりません。M+ 1m は等幅で、heavy と black がない
ため、SemiBold、Bold、Heavy は同じ拼音ウェイトを使います。
...
| バリアント | スタイル | ウェイト | ベースフォント |
| :-: | :-: | :-: | :-: |
| Mengshen-HanSerif | 宋体 | 7種（ExtraLight〜Heavy） | Source Han Serif + M+ M Type-1 |
| Mengshen-Handwritten | 手書き | Regular のみ | Xiaolai Font + SetoFontSP |

宋体は源ノ明朝の7ウェイトすべてで生成できます。手書きはベースフォントが
1ウェイトしか配布されていないため Regular のみです。
